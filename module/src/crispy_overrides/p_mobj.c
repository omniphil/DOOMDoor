...
#include "doomdef.h"
#include "p_local.h"
#include "action_dispatch.h"   // [tracedoom]
#include "sounds.h"
  ...
  statenum_t	state )
{
	...
	// [tracedoom] WebAssembly checks the signature of an indirect call, so the action is called by its real
	// one (src/action_dispatch.c) instead of always as a three-argument function.
	if (st->action.acv)
	    tracedoom_call_action((void *)st->action.acv, mobj, NULL, NULL);
